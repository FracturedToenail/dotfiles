#+TITLE: Leo's Doom Emacs Config
#+AUTHOR: Leo Zamoldinov
#+DESCRIPTION: Leo's personal Doom Emacs config based on DT's config.
...
* DOOM THEME
Setting the theme to doom-moonlight.  To try out new themes, I set a keybinding for counsel-load-theme with 'SPC h t'.
...
* LINE NUMBERS
I prefer relative line numbers, but relative numbers dont work for me. visual line numbers work as i expect though.
